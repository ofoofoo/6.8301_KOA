...
@app.function(
    ...
    secrets=[modal.Secret.from_name("MoEViT")]
)
...
@app.function(image=axolotl_image, volumes=VOLUME_CONFIG, timeout=3600 * 24, secrets=[modal.Secret.from_name("MoEViT")])
...
@app.function(image=axolotl_image, timeout=60 * 30, volumes=VOLUME_CONFIG, secrets=[modal.Secret.from_name("MoEViT")])
def launch(config_raw: str, data_raw: str):
    ...
    try:
        snapshot_download(model_name, local_files_only=True, token=os.environ["HF_TOKEN"])
